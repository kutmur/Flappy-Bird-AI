...
def run_ai():
    ...
    while not done:
        ...
        obs, reward, done, _, = env.step(action)  # ✅ FIXED unpacking issue
        env.render()

    env.close()
    print("💀 AI Died! Returning to Menu...")
    pygame.time.delay(1000)  # Small delay before returning

    pygame.display.init()  # ✅ Fixes screen closing issue
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:
                    ...
                    if result == "menu":
                        pygame.display.init()
                        screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
                        continue  # ✅ Ensures it loops back to main menu
                    elif result == "quit":
                        pygame.quit()
                        return
